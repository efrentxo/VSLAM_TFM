...
if Modo == 1
    ...
    if step4
        ...
        % pasados
                % buscar cual es la imagen del modelo con el mayor match
        Features = binaryFeatures(vSetKeyFrames.Views.Features{1,1});
        for i=1:vSetKeyFrames.NumViews
        % hacer el match con el modelo
            index_matches_cl{i} = matchFeatures(Features, binaryFeatures(vSetKeyFrames.Views.Features{i,1}), 'Unique', true);
            index_matches_cl_size(i) = size(index_matches_cl{i},1);
        end

        % Usar imagen no consecutiva que tenga mayor numero de matches
        cl_detection_min_imagen = 100;
        cl_min = 10; 
        
        [value, id2] = max(index_matches_cl_size(cl_detection_min_imagen:end));
        id2 = id2 + cl_detection_min_imagen; 
        
        if value > cl_min  % Set threshold for loop closure
            loopClosureDetected = true;
            matchedFrameIdx = i;
            disp(['Loop closure detectado entre imagen 1 y la imagen ', num2str(ind)])
        end
        ...
        if LoopClosureDetected 
            id1 = 1;
%           id2 = 418; % Q&D manual loop closure para chequear como queda el mapa3d
    ...
    %% plots intermedios imagen a imagen
    puntos3D = [];
    for i=1:vSetKeyFrames.NumViews
        % crear plot con el mapa actual 
        % puntsos 3d visto del frame X
        % plotar puntos 3D
        % plot posicion camara
        % polot trayectoria
        figure; 
        puntos3D = [puntos3D; findWorldPointsInView(mapPointSet,i)];
        trayectoria = [trayectoria; vSetKeyFrames.Views.AbsolutePose(i,1).Translationlacion];


        camara = vSetKeyFrames.Views.AbsolutePose(i,1).Translationlacion

        sufix = name;
        plot(puntos3D(1,:), puntos3D(2,:))
        plot_resultados_intermedios(vSetKeyFrames, mapPointSet, pathOutputs, sufix)
    

    end
    
    %% PLOTS FINAL
    % NOTE: rectangle('Position',[0 0 6 3])%coordenadas de posicion en plano y dimensiones
    % 
    rectangle('Position',[-0.7 -3 6 4])%coordenadas de posicion en plano y dimensiones
    sufix = 'SinLC';
    plot_resultados(vSetKeyFrames, mapPointSet, pathOutputs, sufix)
    sufix = 'ConLC';
...
elseif Modo ==2
%     Imagen_localizar = '/home/efren/Escritorio/TFM/datos/Crazyflie_toma4/Localizacion/All_ORB_points_img_000075.png';
    load(Modelo_path)
    disp(['Model 3D cargado'])

    imds_localziar = imageDatastore(Imagenes_localziar);
    disp(['Imagenes para localizar cargadas: ', num2str(numel(imds_localziar.Files))])

    for i=1:size(imds_localziar.Files,1)
        currI = readimage(imds_localziar, i);
        
        % Corregir imagen
        img_loc_corr{i}  = undistortImage(currI, intrinsics);

        % extraer ORB featuers
        [Features, Points, AlPoints] = DetectAndExtractFeatures(img_loc_corr{i}, scaleFactor, numLevels, numPoints, intrinsics); 

        % buscar cual es la imagen del modelo con el mayor match
        for j=1:vSetKeyFrames_LoopClosure_Opt.NumViews
        % hacer el match con el modelo
            index_matches_loc{j} = matchFeatures(Features, binaryFeatures(vSetKeyFrames_LoopClosure_Opt.Views.Features{j,1}), 'Unique', true);
            index_matches_loc_size(j) = size(index_matches_loc{j},1);
        end
        
        % usar la imagen con mayor numero de matches para localizar - imagen referencia
       [value, ind] = max(index_matches_loc_size);

       % Q&D elimino esa imagen para no reusarla
       index_matches_loc_size(ind) =[];
       ind = ind -1;

       % buscar matches en la imagen a localizar
       indexpair = index_matches_loc{ind};
       matchedImagePoints = Points(indexpair(:,1));

       % encontrart los matches en el mapa 3D cargado
        % Match features from the previous key frame with known world locations
       [index3d, index2d]    = findWorldPointsInView(mapPoints_LoopClosure_Opt, ind);
       RefImagenKeyFrameFeatures  = vSetKeyFrames_LoopClosure_Opt.Views.Features{ind}(index2d,:);

       RefImagenFramePoints    = vSetKeyFrames_LoopClosure_Opt.Views.Points{ind}(index2d);

       % Repito match con solo los puntos que exiten en el mapa 3D
%        RefFinal_index_matches_loc = matchFeatures(Features, binaryFeatures(RefImagenKeyFrameFeatures), 'Unique', true, 'MaxRatio', 0.9, 'MatchThreshold', 40);
       RefFinal_index_matches_loc = matchFeatures(Features, binaryFeatures(RefImagenKeyFrameFeatures), 'Unique', true);


        % Estimate the camera pose
        matchedImagePoints = Points.Location(RefFinal_index_matches_loc(:,1),:);
        matchedWorldPoints = mapPoints_LoopClosure_Opt.WorldPoints(index3d(RefFinal_index_matches_loc(:,2)), :);
        
        matchedImagePoints = cast(matchedImagePoints, 'like', matchedWorldPoints);
        
        [currPose(i), inlier, status] = estworldpose(matchedImagePoints, matchedWorldPoints, intrinsics,'Confidence', 95, 'MaxReprojectionError', 3, 'MaxNumTrials', 1e4);
       
        % plots report
        f1 = figure; 
        plot(index_matches_loc_size,'o-');
        grid on;
        ylabel('matches')
        xlabel('keyframes')
        saveas(f1,[pathOutputs,'/Localizacion_',num2str(i),'.bmp']);
     
        % only image
        figure;
        imshow(img_loc_corr{i});hold on;

        %afigure + ORB
        figure;
        imshow(img_loc_corr{i});hold on;
        plot(Points,'ShowScale',false, showOrientation=false)    
        
        % matches 
        figure;
        imshow(img_loc_corr{i});hold on;
        plot(Points(indexpair(:,1)),'ShowScale',false, showOrientation=false)   
        size(Points(indexpair(:,1)))

        %matches delos puntos 3D
        figure;
        imshow(img_loc_corr{i});hold on;
        plot(Points(RefFinal_index_matches_loc(:,1),:),'ShowScale',false, showOrientation=false)    
        size(Points(RefFinal_index_matches_loc(:,1),:))

        %inliers
        figure;
        imshow(img_loc_corr{i});hold on;
        plot(Points(RefFinal_index_matches_loc(inlier,1),:),'ShowScale',false, showOrientation=false)   
        size(Points(RefFinal_index_matches_loc(inlier,1),:))
    end

    % plotar mapa y localizacion de la imagen analziada
    sufix = '';
    plot_resultados_localziacion(currPose(2), mapPoints_LoopClosure_Opt, pathOutputs, sufix)
